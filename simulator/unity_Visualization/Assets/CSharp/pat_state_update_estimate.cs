...
public class pat_state_update_estimate : MonoBehaviour
{
  public static int num_joint = 6;
  public static int num_leg = 2;
  ...
  void Update()
  {
    ...
    for(int joint_idx = 0; joint_idx< num_joint; ++joint_idx){
      ...
      //if(joint_idx == 4) { angle[0] = -jpos[joint_idx]*180f/(float)Math.PI; }
  ...
  internal class SimpleSubscriber : LCM.LCM.LCMSubscriber
  {
    ...
    {
      if (channel == "pat_state_est_info")
      {
        LCMTypes.pat_state_info_lcmt msg =
          new LCMTypes.pat_state_info_lcmt(dins);
  ...
  {
    Debug.Log ("Pat listener started.");
    ...
    Debug.Log ("Pat State Update listener coroutine returning!");
  }
