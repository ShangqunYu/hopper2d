...
obs,_ = env.reset()
model = PPO.load("./logs/optijumpJuly11/best_modelPPO.zip", print_system_info=True)
done = False
count = 0
totalReward = 0
while not done:
    ...
    action[4] =  act[4] * 0.2  + 0.3  # range from 0.0 to 0.8
    action[5] =  act[5] * 0.25  + 0.55  # range from 0.3 to 0.8
    breakpoint()
